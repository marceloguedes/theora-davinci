...
//#include "getopt.h"
#include "../theora-DM6446/theora.h"

#define IN_FILE "C:\\sample\\Picasa3-123frames.OGG"
#define IN_FILE_SIZE 123 * 4096
#define OUT_FILE "C:\\sample\\picasa3.out"
...
/* Helper; just grab some more compressed bitstream and sync it for
   page extraction */
int pos = 0;
int buffer_data(char *in,ogg_sync_state *oy){
  int bytes = 4096;
  char *buffer;
  if (pos >= IN_FILE_SIZE) bytes = 0;
  buffer=ogg_sync_buffer(oy,bytes);
  memcpy(buffer, &in[pos], bytes); 
  pos += bytes;
...
unsigned int outbuffer[800*600];
...
static void video_write(void){
  ...
  for(i=0;i<yuv.y_height;i++)
	memmove(outbuffer, yuv.y+yuv.y_stride*i, yuv.y_width);
  for(i=0;i<yuv.uv_height;i++)
    memmove(outbuffer, yuv.u+yuv.uv_stride*i, yuv.uv_width);
  for(i=0;i<yuv.uv_height;i++)
    memmove(outbuffer, yuv.v+yuv.uv_stride*i, yuv.uv_width);
...
//  int c;
  ...
  bytes=fread(huge_buffer,1,IN_FILE_SIZE,infile);
  printf("Ready\n");
  ...
  */
  ...
  while(!stateflag){
    int ret=buffer_data(huge_buffer,&oy);
    if(ret==0)break;
  ...
  while(theora_p && theora_p<3){
    ...
    if(ogg_sync_pageout(&oy,&og)>0){
      ...
      int ret=buffer_data(huge_buffer,&oy); /* need more data */
      if(ret==0){
  ...
  */
  ...
  while(!got_sigint){
    ...
    if(!videobuf_ready && pos >= IN_FILE_SIZE)break;
